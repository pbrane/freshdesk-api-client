...
#  -X PUT "$FD_BASE_URI/tickets/7" |jq

#Add two attachments to Ticket 11 in the Molex sandbox
curl -v -u "$FD_API_KEY":X \
  -F "attachments[]=@./bluey.png" \
  -F "attachments[]=@./Bluey-Wallpaper.jpg" \
  -X PUT "$FD_BASE_URI/tickets/98" |jq > sandbox_ticket_98_add_attachments.json
...
#  -F "status=2" \
#  -F "priority=1" \
#  -X POST "$FD_BASE_URI/tickets" |jq

##Delete an Attachment
#curl -v -u "$FD_API_KEY":X -X DELETE "$FD_BASE_URI/attachments/1"

##Try to list attachments, this probably won't work ;)... It definitely doesn't work.
#curl -v -u "$FD_API_KEY":X -X GET "$FD_BASE_URI/attachments/153035620444"